...
// CommandRunner is a fake easyshift.CommandRunner. All commands succeed and
...
type CommandRunner struct {
	...
	// PoolInfoErr, if set, fails `virsh pool-info` (simulates a missing pool).
	PoolInfoErr error
	// PoolInfoState overrides the State reported by `virsh pool-info`.
	// Empty defaults to "running" so happy-path tests pass.
	PoolInfoState string
...
func (c *CommandRunner) Run(_ context.Context, name string, args ...string) ([]byte, error) {
	...
	if c.Failures != nil {
		if err, ok := c.Failures[name]; ok {
			return nil, err
		}
	}
	if name == "virsh" && argsContain(args, "pool-info") {
		if c.PoolInfoErr != nil {
			return nil, c.PoolInfoErr
		}
		state := c.PoolInfoState
		if state == "" {
			state = "running"
		}
...
// Downloader is a fake easyshift.Downloader. All downloads succeed and
...
// VMManager is a fake easyshift.VMManager. Created VMs are tracked in Created
// and considered running until Stop/Delete is called.
type VMManager struct {
	mu           sync.Mutex
	Created      []easyshift.VMSpec
	Started      []string
	Stopped      []string
	Deleted      []string
...
func (v *VMManager) Create(_ context.Context, spec easyshift.VMSpec) error {
	v.mu.Lock()
	defer v.mu.Unlock()
	if v.running == nil {
...
// NetworkProvisioner is a fake easyshift.NetworkProvisioner.
type NetworkProvisioner struct {
	mu      sync.Mutex
	Created []easyshift.NetworkSpec
	Deleted []string
	Err     error
}

// CreateNetwork records the spec.
func (n *NetworkProvisioner) CreateNetwork(_ context.Context, spec easyshift.NetworkSpec) error {
	n.mu.Lock()
	defer n.mu.Unlock()
...
// Installer is a fake easyshift.Installer. LastSpec captures the spec from
...
type Installer struct {
	...
	LastSpec               easyshift.InstallerSpec
	// LiveISOURL overrides the URL returned by CoreOSLiveISOURL.
	LiveISOURL string
	Err        error
}

func (i *Installer) record(spec easyshift.InstallerSpec) {
	i.LastSpec = spec
}

func (i *Installer) WriteInstallConfig(_ context.Context, spec easyshift.InstallerSpec) error {
	i.mu.Lock()
	defer i.mu.Unlock()
...
func (i *Installer) CreateIgnitionConfigs(_ context.Context, spec easyshift.InstallerSpec) error {
	i.mu.Lock()
	defer i.mu.Unlock()
...
func (i *Installer) CreateSingleNodeIgnition(_ context.Context, spec easyshift.InstallerSpec) error {
	i.mu.Lock()
	defer i.mu.Unlock()
...
func (i *Installer) EmbedIgnitionInISO(_ context.Context, spec easyshift.InstallerSpec, _, _, _ string) error {
	i.mu.Lock()
	defer i.mu.Unlock()
	i.EmbeddedISO = true
	i.record(spec)
...
func (i *Installer) WaitForInstallComplete(_ context.Context, spec easyshift.InstallerSpec) error {
	i.mu.Lock()
	defer i.mu.Unlock()
...
func (i *Installer) CoreOSLiveISOURL(_ context.Context, spec easyshift.InstallerSpec) (string, error) {
	i.mu.Lock()
	defer i.mu.Unlock()
	i.record(spec)
	if i.Err != nil {
...
// FileServer is a fake easyshift.FileServer.
...
// CSRApprover is a fake easyshift.CSRApprover. Run records that it was
...
// CertIssuer is a fake easyshift.CertIssuer. It records the domain sets
...
type CertIssuer struct {
	...
	LastOpts easyshift.CertIssuerOpts
	Err      error
}
...
func (c *CertIssuer) recordOpts(o easyshift.CertIssuerOpts) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.LastOpts = o
}

// DNSManager is a fake easyshift.DNSManager. Records every Upsert/Delete
...
// HostnameInjector is a fake easyshift.HostnameInjector. Run records the
...
// HostInspector is a fake easyshift.HostInspector. By default every
...
type HostInspector struct {
	...
	Bridges map[string]easyshift.BridgeInfo
...
func (h *HostInspector) InspectBridge(name string) (easyshift.BridgeInfo, error) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if h.Err != nil {
		return easyshift.BridgeInfo{}, h.Err
	}
	if h.MissingBridges[name] {
		return easyshift.BridgeInfo{Exists: false}, nil
	}
	if info, ok := h.Bridges[name]; ok {
		return info, nil
	}
	return easyshift.BridgeInfo{Exists: true, Slaves: []string{"eth0"}, Up: true}, nil
}
...
// DNSResolver is a fake easyshift.DNSResolver. Records maps name → IPs;
...
func All() (easyshift.Deps, *Bundle) {
	b := &Bundle{
		Cmd:        &CommandRunner{},
		Download:   &Downloader{},
		VM:         &VMManager{},
	...
	return easyshift.Deps{
		...
		NewCertIssuer: func(opts easyshift.CertIssuerOpts) (easyshift.CertIssuer, error) {
