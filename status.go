...
// Status runs read-only diagnostics against the cluster: VM state, master
// reachability (bridge mode), DNS records, API port, plus the tail of the
// openshift-install log. It does not mutate any cluster state.
...
func (cm *ClusterManager) checkVMState(ctx context.Context, c *ClusterConfig) StatusCheck {
	vmName := fmt.Sprintf("master-0-%s", c.Name)
	out, err := cm.deps.Cmd.Run(ctx, "virsh", "-c", LibvirtSystemURI, "domstate", vmName)
	if err != nil {
		return StatusCheck{
			Name:   "VM exists",
...
func (cm *ClusterManager) checkARP(c *ClusterConfig) StatusCheck {
...
func (cm *ClusterManager) checkDNS(ctx context.Context, c *ClusterConfig, fqdn string) StatusCheck {
	var bad []string
...
// installerLogPath is the openshift-install log produced inside the cluster dir.
func installerLogPath(configDir, clusterName string) string {
	return configDir + "/clusters/" + clusterName + "/.openshift_install.log"
}

// tailFile returns the last n lines of path, or "" if it doesn't exist.
