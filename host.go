...
func (SystemHostInspector) InspectBridge(name string) (BridgeInfo, error) {
	base := filepath.Join("/sys/class/net", name)
	if _, err := os.Stat(filepath.Join(base, "bridge")); err != nil {
		if os.IsNotExist(err) {
			return BridgeInfo{Exists: false}, nil
		}
		return BridgeInfo{}, err
	}
	info := BridgeInfo{Exists: true}
	entries, err := os.ReadDir(filepath.Join(base, "brif"))
	if err != nil && !os.IsNotExist(err) {
		return BridgeInfo{}, fmt.Errorf("read brif for %s: %w", name, err)
	}
	...
	if err != nil {
		return BridgeInfo{}, fmt.Errorf("read operstate for %s: %w", name, err)
	}
