...
func TestCoreOSLiveISOURL_ParsesStreamJSON(t *testing.T) {
	...
	installer := easyshift.NewOpenShiftInstaller(cmd)

	url, err := installer.CoreOSLiveISOURL(context.Background(), easyshift.InstallerSpec{
...
func TestWriteInstallConfig_RendersBootstrapInPlace(t *testing.T) {
	...
	spec := easyshift.InstallerSpec{
		ClusterDir: dir,
		Cluster: &easyshift.ClusterConfig{
			Name:        "demo",
			Domain:      "local",
			MasterCount: 1,
			WorkerCount: 0,
	...
	for _, want := range []string{
		"bootstrapInPlace:",
		"installationDisk: " + easyshift.SNOInstallationDisk,
		"controlPlane:",
		"replicas: 1",
		"replicas: 0",
		"baseDomain: local",
