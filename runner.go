...
func (r *Runner) Preflight(ctx context.Context, sc *StageContext, stages []Stage) error {
	var errs []error
...
func (r *Runner) Apply(ctx context.Context, sc *StageContext, stages []Stage) error {
	...
	for _, s := range stages {
		...
		state.Stages[s.Name()] = StageRecord{
			AppliedAt: r.now(),
...
func (r *Runner) Rollback(ctx context.Context, sc *StageContext, stages []Stage) error {
	state, err := r.loadState(sc.Cluster.Name)
	if err != nil {
		return fmt.Errorf("load state: %w", err)
	}
...
func (r *Runner) loadState(clusterName string) (*ClusterState, error) {
	path := r.statePath(clusterName)
	data, err := os.ReadFile(path)
	if errors.Is(err, os.ErrNotExist) {
		return &ClusterState{Stages: map[string]StageRecord{}}, nil
	}
	if err != nil {
		return nil, err
	}
...
func (r *Runner) saveState(clusterName string, state *ClusterState) error {
	path := r.statePath(clusterName)
